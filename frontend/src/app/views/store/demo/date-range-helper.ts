...
export class DateRangeHelper {
  ...
  static getDates = (startDate: any, stopDate: any): Array<Date> => {
    ...
    if (stopDate - startDate >= 0) {
      ...
      console.log(
        '*** stopDate - startDate is not > 0 meaning dates are possibly inversed'
      ); // keep as error message
      return;
    }
  };
   ...
   */
  static dateRangeDaysCalculator = (
    stopDate: number,
    startDate: number
  ): number => {
  ...
  static dateRangeMonthsCalculator = (arrivalDate: number): number => {
    let dateRangeMonths =
      new Date(arrivalDate).getMonth() - new Date().getMonth();
