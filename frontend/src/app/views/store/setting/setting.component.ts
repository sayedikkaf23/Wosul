...
export class SettingComponent implements OnInit {
  ...
  ngOnInit() {
        ...
        (res_data: any) => {
          if (res_data.success == false) {
            ...
            localStorage.setItem(
              'store',
              JSON.stringify(res_data.store_detail)
            );
            ...
            this.store_setting.auto_complete_order = store.auto_complete_order;
            this.change_day(0);
    ...
    //        jQuery(document.body).find('#delivery_time').on('change', (evnt,res_data) => {
    //
  ...
  add_store_time() {
    ...
    this.selected_store_time_array.push({
      store_open_time: this.store_setting.store_open_time,
      store_close_time: this.store_setting.store_close_time,
    });
