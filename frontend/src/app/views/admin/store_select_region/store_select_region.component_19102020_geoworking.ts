...
export class StoreSelectRegionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.helper.router_id.admin.store_setting.delivery_regions == null) {
      var location = this.helper.router_id.admin.store_location;
      var squareCoordinates = this.createPolygon(location[0], location[1], 10);
      ...
      var regions = this.helper.router_id.admin.store_setting.delivery_regions;
      var region = regions.coordinates[0];

      var coordinates = [];
      for (var i = 0; i < region.length; i++) {
        var position = region[i];
        coordinates.push(new google.maps.LatLng(position[1], position[0]));
      }
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      var location = this.helper.router_id.admin.store_location;
      this.map.setCenter(new google.maps.LatLng(location[0], location[1]), 13);
