...
export class StoreSelectRegionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.store_setting.radius_regions.length > 0) {
      for (var i = 0; i < this.store_setting.radius_regions.length; i++) {
        var a = this.store_setting.radius_regions[i];
        var coordinates = [];
        for (var j = 0; j < a.kmlzone.length; j++) {
          var b = a.kmlzone[j];
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      this.locationPolygon.forEach((arra) => {
      ...
      google.maps.event.addListener(
        ...
        (polygon) => {
          ...
          shape.index = this.radius_regions.length;
          ...
          var html =
            ...
            '<div><input type="text" id="price" name="price"/></div><br>' +
            '<div><input type="text" id="timeMin" name="timeMin"/></div><br>' +
            '<div><input type="text" id="timeMax" name="TimeMax"/></div><br>' +
            ...
            jQuery('#submit_title').on('click', (event, res_data) => {
              if (this.selectedShape) {
                if (jQuery('#title').val() != '') {
                  ...
                  this.radius_regions[this.selectedShape.index].title =
                    jQuery('#title').val();
  ...
  onSave() {
    ...
    this.helper.router_id.admin.store_setting.radius_regions =
      this.radius_regions;
