...
export class AddIngredientComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.helper.router_id.admin.store_id = store?._id;
    ...
    this.storeService
      .getAdminStoreSpecificationGroup({
        type: this.helper.ADMIN_DATA_ID.ADMIN,
    ...
    this.storeService
      ...
      .subscribe((data: any) => {
        ...
        //       console.log(
        //         ' this.selected_category_for_product : ',
        //         this.selected_category_for_product
        //       );
        //     });
        // }, 1000);
  ...
  onItemSelect(event) {
    ...
    if (
      confirm(
        `Are you sure you want to change the Category to ${event.name} for this Item?`
      )
    ) {
      this.itemService
        .updateSubCategory({
  ...
  onCategoryChange(product_id) {
    this.selected_category_for_product = product_id;
  }
  ...
  addProduct() {
    if (!this.product_already_exist && this.add_product.name.trim() != '') {
      if (this.selected_category_for_product == '') {
        ...
        this.storeService.addStoreProduct(this.add_product).subscribe(
          (res_data: any) => {
            this.addproductform = false;
  ...
  deleteProduct(product_data) {
    ...
    this.myLoading = true;
    this.storeService.deleteStoreProduct(json).subscribe(
      (res_data: any) => {
        this.myLoading = false;
        if (res_data.success == true) {
          this.helper.data.storage = {
  ...
  deleteItem(item_data) {
    ...
    this.myLoading = true;
    this.storeService.deleteStoreItem(json).subscribe(
      (res_data: any) => {
        this.myLoading = false;
        if (res_data.success == true) {
          this.helper.data.storage = {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (index !== -1) {
        ...
        reader.onload = (e: any) => {
          ...
          this.base64Image = new_image.src;
  ...
  updateItem() {
    ...
    this.storeService.updateStoreItem(this.item_detail).subscribe(
      (res_data: any) => {
        this.filtered_item_list.sort(this.sortItem);
        if (res_data.success == true) {
          this.helper.data.storage = {
  ...
  addStoreIngredient() {
    const payload = {
      ...
      unique_id_for_store_data: this.itemBarcode,
      is_item_in_stock: this.isItemInStock,
      item_name_arabic: this.itemNameArabic,
      ...
      measurement: this.selectedMeasurment,
      brand: this.selectedBrand,
      manufacturer_date: this.manufacturerDate,
  ...
  clearFields() {
    ...
    this.itemNameArabic = '';
    this.purchaseCost = '';
    this.salePriceExclude = 0;
    this.salePriceInculde = 0;
