                  ...
                  Order.aggregate([
                    ...
                    array_to_json1,
                    {
                      $lookup: {
                        ...
                        foreignField: "_id",
                        pipeline: [ {$project : {payment_mode : {
                          $switch : {
                            branches : [
                              { "case": "$is_payment_mode_online_payment", "then": "Online" },
                              { "case": "$is_payment_mode_card_on_delivery", "then": "Card on Delivery" }
                            ],
                            "default": "Cash"
                          }
                        }} } ],
                    ...
                    { $addFields: { payment_gateway_name: "$payment_gateway_name.payment_mode" } }
                  ]).then(
                    (new_order_list) => {
                      var lookup = {
                        $lookup: {
                          ...
                          Order.aggregate([
                            ...
                            async(orders) => {
                              if (orders.length === 0) {
                                ...
                                const is_show_user_info =  store.is_show_user_info == undefined || null ? false : store.is_show_user_info
                                orders.forEach((order)=>{
                                  if(!is_show_user_info){
                                    let email = order.user_detail.email.split("@")
                                    order.user_detail.phone = "XXXXXXX" + order.user_detail.phone.slice(6)
                                    order.user_detail.email = "xxxxxxx@" +  email[email.length-1]
                                    order.user_detail.first_name = Date.now();
                                    order.cart_detail.destination_addresses[0].user_details.phone = "XXXXXXX" + order.user_detail.phone.slice(6)
                                    order.cart_detail.destination_addresses[0].user_details.name = Date.now()
                                  }
                                })
                                response_data.json({
                                  success: true,
